...
const withPWA = require('next-pwa')({
  ...
  additionalManifestEntries: (() => {
    ...
    try {
      ...
      const routesFilePath = path.join(__dirname, 'public', 'routesprecache.json');
      ...
      if (fs.existsSync(routesFilePath)) {
        ...
        console.log('Archivo routesprecatch.json no encontrado, usando rutas predeterminadas');
      }
    } catch (error) {
      console.error('Error al leer el archivo de rutas para precaching:', error);
    }
...
return routes.map(url => ({ url, revision: BUILD_DATE }));
  ...
  runtimeCaching: [ // Configuración del cacheo en tiempo de ejecución (para recursos dinámicos)
    {
