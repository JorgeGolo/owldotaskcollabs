...
export async function getStaticPathsPRO() {
  ...
  try {
    const res = await fetch('https://8txnxmkveg.us-east-1.awsapprunner.com/api/frontend/quizzes-paths');
...
export async function getStaticProps({ params }) {
  ...
  const chapters = chaptersSnapshot.docs.map((doc) =>
    serialize({ id: Number(doc.data().id), ...doc.data() })
  ).sort((a, b) => a.id - b.id);

  const fichalibroRef = collection(doc(db, "quizzes", quizDoc.id), "fichalibro");
  const fichalibroSnapshot = await getDocs(fichalibroRef);
  const fichalibro = fichalibroSnapshot.empty ? null : serialize(fichalibroSnapshot.docs[0].data());
...
const Questionnaire = ({ quiz, chapters, fichalibro, category }) => {
  ...
  const breadcrumbSegments = [
    ...
    const shareMessage = "Check out OwldoTask and earn feathers by completing tasks!";
  ...
  return (
    <div className="flex flex-col lg:flex-row">
      ...
      <title>{`${quiz.title} | Quiz | OwldoTask`}</title>
        ...
        {/* Open Graph / Facebook */}
        <meta property="og:title" content={`${quiz.title} | Quiz | OwldoTask`} />
        <meta
        ...
        <meta name="twitter:title" content={`${quiz.title} | Quiz | OwldoTask`} />
        <meta
      ...
      <div className="p-4">
        ...
        <div className="mt-2 mb-2">
          ...
          <div className="pl-2 flex items-center bg-[#fef9c3] border border-[#eab308] rounded-md text-[#3f2e1f]">
            ...
            <div className="flex items-center space-x-2 p-2">
                ...
                {shareOnSocial("reddit", currentUrl, shareMessage)} {/* ¡Botón de Reddit aquí! */}
            </div>
        ...
        {!isReliablyOnline ? (
          ...
          <Test chapters={chapters} title={quiz.title_slug} numberOfQuestions={quiz.numberOfQuestions} />
        }
            ...
            <motion.div className="border border-1 border-green-200 flex flex-col md:flex-row p-4 mb-2 bg-green-50 rounded-lg shadow hover:shadow-lg transition">
              {fichalibro.Cover && (
                <div className="flex-shrink-0 mb-4 md:mb-0 md:mr-4">
                  <img src={fichalibro.Cover} alt="Cover" className="w-full md:w-48 h-auto rounded" />
                </div>
              )}
              <div className="flex-grow">
                <ul className="text-gray-800">
                  {fichalibro.Title && <li className="mb-2"><strong>Title:</strong> {fichalibro.Title}</li>}
                  {fichalibro.Author && <li className="mb-2"><strong>Author:</strong> {fichalibro.Author}</li>}
                  {fichalibro.Year && <li className="mb-2"><strong>Year:</strong> {fichalibro.Year}</li>}
                  {fichalibro.Pages && <li className="mb-2"><strong>Pages:</strong> {fichalibro.Pages}</li>}
                  {fichalibro.Editions && <li className="mb-2"><strong>Editions:</strong> {fichalibro.Editions}</li>}
                  {fichalibro.Description && <li className="mb-2"><strong>Description:</strong> {fichalibro.Description}</li>}
                </ul>
              </div>
            </motion.div>
          </div>
