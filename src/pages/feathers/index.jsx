...
import { faCalendar, faTasks, faGamepad, faTrophy, faCaretDown, faCaretRight  } from '@fortawesome/free-solid-svg-icons';
...
const Feathers = () => {
  ...
  const { user, clientData, featherIcon, levelData, refreshClientData, checkCanEarnPoints, getOfflineMessage } = useContext(AppClientContext);
  ...
  useEffect(() => {
    if (localClientData && localClientData.totalfeathers !== undefined) {
      const calculatedLevelProgress = Math.min((localClientData.totalfeathers / localLevelData[localClientData.level - 1].tolevelup) * 100, 100);
  ...
  const claimFeathers = async () => {
    ...
    try {
      ...
      const response = await fetchWithToken(`https://8txnxmkveg.us-east-1.awsapprunner.com/api/makePayment2`, {
        method: "POST",
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-8">
            ...
            <div className="lg:mb-6 md:mb-6 mb-0 p-4 bg-gray-50 rounded-md shadow-inner">
              <h2 className="flex text-2xl font-bold text-gray-800 mb-2 lg:mb-4">
                ...
                </span> Level {localClientData.level}
              </h2>
              
                <div className="border p-2 rounded-md bg-white">
                  <div className="font-bold italic">Benefits</div>
                  <p>
                    Quizzes per day: {localClientData.quizzestoday || '0'} / {localLevelData[localClientData.level - 1]?.maxquizzesperday || '0'} 
                  </p><p>
                    Games per day: {localClientData.gamestoday || '0'} / {localLevelData[localClientData.level - 1]?.maxgamesperday || '0'} 
                  </p>
                  <p>
              ...
              <div className="border p-2 rounded-md bg-white mt-2">
                ...
                <p>
                  Quizzes finished: {localClientData.quizzesdone} / {localLevelData[localClientData.level - 1].quizzestolevelup} 
                  {localLevelData[localClientData.level - 1].quizzestolevelup <= localClientData.quizzesdone && 
                    <span className="mx-2 text-green-500 font-bold italic text-sm">Done!</span>
                  }
                </p>
                <p>
                  Games finished: {localClientData.gamesdone} / {localLevelData[localClientData.level - 1].gamestolevelup}
                  {localLevelData[localClientData.level - 1].gamestolevelup <= localClientData.gamesdone && 
                    <span className="mx-2 text-green-500 font-bold italic text-sm">Done!</span>
                  }
                </p>
                
                <p>Feathers to level up: {localClientData.totalfeathers} / {localLevelData[localClientData.level - 1].tolevelup}</p>
                <div className="w-full bg-gray-300 rounded-full h-2.5 mt-2">
                  <div
                    className="bg-teal-600 h-2.5 rounded-full"
                    style={{ width: `${progressLevel}%`, transition: 'width 1s ease-in-out' }}
                  ></div>
                </div>
              </div>
              ...
              <p className="hidden">{localLevelData && JSON.stringify(localLevelData)}</p>
            </div>
            
            {/* Flecha vertical para móvil */}
            <div className="flex justify-center lg:hidden">
              <FontAwesomeIcon icon={faarrowdown} className="text-gray-300 text-4xl" />
            </div>
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-md shadow-inner">
              <h2 className="flex text-2xl font-bold text-gray-800 mb-2 lg:mb-4">
                <span className="text-yellow-500 inline-flex items-center mr-2">
                  <FontAwesomeIcon icon={falevel} />
                </span>Level {localClientData.level+1}
              </h2>
              <div className="border p-2 rounded-md">
                <div className="font-bold italic">Benefits</div>
                <p>Quizzes per day: {localLevelData[localClientData.level]?.maxquizzesperday || 'N/A'}</p>
                <p>Games per day: {localLevelData[localClientData.level]?.maxgamesperday || 'N/A'}</p>
                <p>
                  {4 + localClientData.level +1} <b>feathers</b> per quiz completed
                </p>
                <p>
                  {10 + localClientData.level +1} <b>feathers</b> per game completed
                </p>
