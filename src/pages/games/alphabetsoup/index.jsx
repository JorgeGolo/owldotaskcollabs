...
const SopaDeLetras = () => {
  ...
  const palabrasPosibles = [
    'HOUSE', 'MOTOR', 'BOOK', 'TRAIN', 'SUN', 'LIGHT', 'WATER', 'FIRE', 
    'EARTH', 'AIR', 'CAR', 'BOAT', 'PAPER', 'PEN', 'FLOWER', 'TREE', 
    'TABLE', 'CHAIR', 'LAPTOP', 'KEYBOARD', 'MOUSE', 'SCREEN', 'FRAME', 
    'CLOCK', 'PLANT', 'KITCHEN', 'DOOR', 'GARDEN', 'ROAD', 'BEACH', 'SKY', 
    'GROUND', 'FOREST', 'RIVER', 'SEA', 'WALL', 'WINDOW', 'DRAWER', 
    'DRINK', 'INKPOT', 'TICKET', 'PANTS', 'SHOES', 'HAT', 'SCARF', 'BOTTLE', 
    'GLASS', 'KNIFE', 'OVEN', 'CARD', 'MONEY', 'MUSEUM', 'THEATER', 'CINEMA', 
    'COFFEE', 'MILK', 'CHEESE', 'BREAD', 'GUITAR', 'DRUM', 'BALCONY', 'PILLOW', 
    'SHIRT', 'SKIRT', 'JACKET', 'COAT', 'STONE', 'TRUCK', 'MARCH', 'TRAVEL', 
    'PLANE', 'BIKE', 'DREAM', 'CLOUD', 'STAR', 'PLANET', 'SWORD', 'HELMET', 
    'SHIELD', 'RING', 'NECKLACE', 'EARRING', 'GAME', 'TOWER', 'CASTLE', 
    'BRIDGE', 'PAINT', 'BRUSH', 'BRICK', 'CANDLE', 'CANOPY', 'CARPET', 
    'CART', 'CHALK', 'CLOAK', 'CUPBOARD', 'CUSHION', 'DESK', 'DOCK', 'DOLLAR', 
    'DRESS', 'DRIVER', 'ENGINE', 'FENCE', 'FLOOR', 'FORK', 'FRIDGE', 'FRUIT', 
    'GLOBE', 'GOLD', 'HAMMER', 'HANDLE', 'HANGER', 'HEATER', 'HILL', 'ISLAND', 
    'JEANS', 'JUG', 'KETTLE', 'LADDER', 'LAMP', 'LEMON', 'LOCKER', 'MARKER', 
    'MIRROR', 'MUG', 'NAPKIN', 'NEEDLE', 'NIGHT', 'OCEAN', 'OFFICE', 'ORANGE', 
    'PALACE', 'PENCIL', 'PLATES', 'PORTAL', 'POSTER', 'POTATO', 'POUCH', 'QUARTZ', 
    'QUILL', 'RACKET', 'RAILWAY', 'REMOTE', 'ROCKET', 'ROOF', 'ROOM', 'SADDLE', 
    'SALAD', 'SANDAL', 'SCHOOL', 'SEAT', 'SHELF', 'SHELL', 'SINK', 'SOCKET', 
    'SPOON', 'STAPLE', 'STATUE', 'STICKER', 'STOVE', 'SUITCASE', 'SWITCH', 
    'TENT', 'FOSSIL', 'GOWN', 'LADLE', 'PEBBLE', 'SPEAR', 'VASE', 'BLADE', 
    'PIGEON', 'POTION', 'TUNNEL', 'HARBOR', 'ANCHOR', 'TUNICS'
  ];
  ...
  const seleccionarPalabrasAleatorias = (listaPalabras, cantidad) => {
    const palabrasFiltradas = listaPalabras.filter(p => p.length >= 4 && p.length <= GRID_SIZE);
    const seleccionadas = [];
  ...
  const crearSopaDeLetras = (palabrasParaColocar, tamano) => {
    const grid = Array(tamano).fill(null).map(() => 
      Array(tamano).fill(null).map(() => ({
        letra: '',
        palabraIndices: [],
        esPalabra: false,
      }))
    ...
    for (let y = 0; y < tamano; y++) {
      for (let x = 0; x < tamano; x++) {
        ...
        celda.palabraIndices = celda.palabraIndices.map(idx => mapaIndices[idx]).filter(idx => idx !== undefined);
        if (!celda.esPalabra) {
          celda.letra = String.fromCharCode(65 + Math.floor(Math.random() * 26));
        }
  ...
  const iniciarJuego = () => {
    ...
    try {
      ...
      const { nuevaGrid, palabrasFinales } = crearSopaDeLetras(palabrasSeleccionadas, GRID_SIZE);
  ...
  const handleMouseOver = (rowIndex, colIndex) => {
    if (!seleccion.inicio || (seleccion.fin && seleccion.fin.row === rowIndex && seleccion.fin.col === colIndex)) return;
    ...
    if (diferenciaX === 0 && diferenciaY === 0) {
        // No movement yet
    } else if (diferenciaX === 0) { // Vertical
      direccion = { dx: 0, dy: Math.sign(diferenciaY) };
    } else if (diferenciaY === 0) { // Horizontal
      direccion = { dx: Math.sign(diferenciaX), dy: 0 };
    } else if (Math.abs(diferenciaX) === Math.abs(diferenciaY)) { // Diagonal
    ...
    setSeleccion(prev => ({ ...prev, fin: { row: rowIndex, col: colIndex }, celdas }));
  };
  ...
  const handleMouseUp = () => {
    ...
    const palabraSeleccionada = seleccion.celdas.map(c => grid[c.row][c.col].letra).join('');
  ...
  const estaSeleccionada = (row, col) => seleccion.celdas.some(c => c.row === row && c.col === col);
            ...
            {pointsForGamesError ? (
              ...
              <div>
                { canEarnPointsFromGames === null ? (
                  ...
                  <div className="hidden text-green-500">You can earn more feathers today.</div>
                ) : (
