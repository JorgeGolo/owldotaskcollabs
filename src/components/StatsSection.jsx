...
<div>
    ...
    <section className="grid md:grid-cols-3 gap-6 text-center py-4">
      {stats.map((stat, index) => (
          <div key={index} className={`shadow-lg shadow-gray-500/50 dark:shadow-black p-6 rounded-2xl ${stat.color}`}>
        <span className="text-4xl font-bold text-primary dark:text-white">
          ...
          <p className="mt-2 text-lg text-gray-600 dark:text-white">{stat.label}</p>
        </div>
      ))}
    </section>
