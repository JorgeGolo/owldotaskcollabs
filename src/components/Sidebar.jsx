            ...
            {item.children ? (
              <div className="relative group inline-block">
                <span className="cursor-pointer hover:text-blue-500 flex items-center gap-2 px-2">
                  {item.icon && <FontAwesomeIcon icon={item.icon} className='inline w-16 h-16'/>} {item.label}
                </span>
                <ul className="hidden group-hover:block absolute z-10 bg-white shadow-lg md:static md:shadow-none md:ml-4">
                  {item.children.map((child, childIndex) => (
                    <li key={childIndex} className="mb-1 px-2 py-1 hover:bg-gray-100">
                      <Link href={child.path} className="cursor-pointer hover:text-blue-500 flex items-center gap-2">
                        {child.icon && <FontAwesomeIcon icon={child.icon} className='inline w-16 h-16'/>} {child.label}
                      </Link>
                    </li>
                  ))}
                </ul>
              ...
              <Link href={item.path} className="block mt-1 cursor-pointer hover:text-blue-500 px-2">
                <FontAwesomeIcon icon={item.icon} className='inline w-16 h-16'/> {item.label}
              </Link>
            )}
          </li>
        ))}
