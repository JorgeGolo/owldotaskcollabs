...
const Test = ({ chapters, numberOfQuestions  }) => {
 ...
 const fetchQuestion = useCallback(async () => {
  ...
  try {
    ...
    const response = await fetchWithToken("https://8txnxmkveg.us-east-1.awsapprunner.com/api/groq/generate-question", {
      method: "POST",
      headers: {
        ...
        // mostrar lo siguiente si está autenticado  
          ...
          {pointsError ? (
            ...
            <div>
              ...
              canEarnPoints === 'cargando' ? (
                ...
                <div className="bg-[#fef9c3] border border-[#eab308] p-2 mb-2 italic text-[#705502] rounded-md">
                  <p>You can try more quizzes, but you cannot earn more feathers here today.</p>
          ...
          <div>
            ...
            <div className="mt-4 p-4 border border-gray-300 rounded-lg bg-green-100 text-green-800">
              ...
              <p className="mt-2">You've answered {numberOfQuestions} questions. ¡Congratulations!</p>
              <p className="mt-2 font-bold">✅ Final score: {score} / {numberOfQuestions}</p>
                ...
                {feedback && <p>{feedback}</p>}
                  ...
                  <button
                    ...
                    className={`bg-blue-500 text-white p-2 rounded transition mb-4 mt-4 ${
                      !isAnswered ? "opacity-50 cursor-not-allowed" : "hover:bg-blue-700"
                    }`}
                    disabled={!isAnswered}
                  >
                    {questionCount >= numberOfQuestions - 1 ? "Finish" : "Next Question"}
                  </button>
